  ...
  describe "#fetch_forecast" do
    ...
    context "when the API call is successful" do
      before do
        mock_response = double("HTTP Response",
          ...
          parsed_response: {
            "list" => [
              { "dt" => Time.now.to_i, "main" => { "temp" => 15 }, "weather" => [{ "description" => "clear sky", "icon" => "01d", "id" => 800 }] },
              { "dt" => (Time.now + 1.day).to_i, "main" => { "temp" => 18 }, "weather" => [{ "description" => "few clouds", "icon" => "02d", "id" => 801 }] }
            ]
          }
        )
        allow(HTTParty).to receive(:get).with("/forecast", any_args).and_return(mock_response)
    ...
    context "when the API call fails" do
      ...
      it "handles API errors gracefully" do
        ...
        expect(forecast).to be_a(Hash)
        expect(forecast[:error]).to be_present
        expect(forecast[:error]).to include("401 - {\"cod\":401, \"message\": \"Invalid API key.\"}")
      end
    end
